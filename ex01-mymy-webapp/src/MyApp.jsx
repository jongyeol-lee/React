...
class MyApp extends Component{ //상속을 통해 Component의 능력(멤버들)을 가지게 됨
    ...
    render(){
        ...
        // 이런식으로 .js 안에.. html태그문을 같이 사용할 수 있도록 만든 언어 : JSX언어 (JS + XML[html])
        ...
        return (
            // class 명 지정할때..속성명 class 대신 className 사용을 권장- why? JS에서 class를 설계도를 만드는 키워드
            <div className='wrap'>
                ...
                <input type="text" />
                {/* 속성값도 JS변수로 설정 가능 */}
                <input type={type} id='' readOnly placeholder='sadf' />
                
                {/* JSX언어는 반드시 end tag가 있어야 함. 만약 없애려면 closing tag 사용 */}
                <hr />

                {/* 이미지 보여주기 (별도 수업예정) - 맛보기 */}
                {/* src 폴더에서 작업하는 component의 이미지들은 src폴더안에 위치하는 것을 선호함 */}
